...
def common_assertions(maped_dict: dict[str, object]) -> None:
    ...
    assert (
        cast("dict[str, object]", maped_dict["complex_field"])["nested_int"]
        == EXPECTED_INT_FIELD
    )  # Transformed
    assert (
        cast("dict[str, object]", maped_dict["complex_field"])["nested_str"] == "world"
    )
    ...
    assert (
        cast("list[dict[str, object]]", complex_list[1]["secondary_field"])[0][
            "secondary_int"
        ]
        == EXPECTED_INT_FIELD
    )
    assert (
        cast("list[dict[str, object]]", complex_list[1]["secondary_field"])[3][
            "secondary_str"
        ]
        == "complex"
    )
    assert (
        cast("list[dict[str, object]]", complex_list[1]["secondary_field"])[1][
            "secondary_int"
        ]
        is None
    )
    assert (
        cast("list[dict[str, object]]", complex_list[1]["secondary_field"])[2][
            "secondary_str"
        ]
        == "double complex"
    )
    assert (
        cast("list[dict[str, object]]", complex_list[0]["secondary_field"])[1][
            "secondary_int"
        ]
        == EXPECTED_INT_FIELD
    )  # Transformed
